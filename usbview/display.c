...
{
	UNREFERENCED_PARAMETER(add_text);
    TV_ITEM tvi;
    PVOID   info;
    ULONG   i;
    HRESULT hr = S_OK;
...
{
    HRESULT hr = S_OK;
	//UNREFERENCED_PARAMETER(additional_text);
    ...
    )
{
	...
	WCHAR vol_name[256];
	int in = 0, vol_counter = 0;
	for (int i = 0; ; i++) {
		if (PortConnectorProps->CompanionHubSymbolicLinkName[i] == '{')
		{
			in = 1;
		}
		if (in) 
		{
			vol_name[vol_counter] = PortConnectorProps->CompanionHubSymbolicLinkName[i];
			vol_counter++;
		}
		if (PortConnectorProps->CompanionHubSymbolicLinkName[i] == '}')
		{
			vol_name[vol_counter] = '\0';
			break;
		}
		if (PortConnectorProps->CompanionHubSymbolicLinkName[i] == '\0')
		{
			break;
		}
...
{
	UNREFERENCED_PARAMETER(ConnectionInfoV2);
	...
	*/
    ...
    AppendTextBuffer("USB Signiture:                     0x%02X\r\n",
    ...
    {
        AppendTextBuffer("\r\n          ---===DisplayPipeInfoDisplayPipeInfo>Open Pipes<===---\r\n");
